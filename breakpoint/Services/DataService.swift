...
class DataService {
    ...
    func uploadPost(withMesaage message: String, forUID uid: String, withGroupKey groupKey: String?, sendComplete: @escaping (_ status: Bool) -> ()) {
        if groupKey != nil {
            //sned to group ref
        ...
        REF_GROUPS.observeSingleEvent(of: .value) { (groupSnapshot) in
            ...
            for group in groupSnapshot {
                ...
                if memberArray.contains((Auth.auth().currentUser?.uid)!) {
                    ...
                    let groupModel = Group(title: title, description: description, key: group.key, members: memberArray, memberCount: memberArray.count)
                    groupsArray.append(groupModel)
                }
            }
            print(groupsArray)
            handler(groupsArray)
